...
class Connection:
    ...
    def user_info(self):
        ...
        """
        BasicTeam = namedtuple('BasicTeam', ['league_name', 'league_key', 'team_name', 'team_key'])
        ...
        if number_teams == 1:
            teams = [BasicTeam(league_name=d_league['leagues']['league']['name'],
                               league_key=d_league['leagues']['league']['league_key'],
                               team_name=d_team['teams']['team']['name'], team_key=d_team['teams']['team']['team_key'])]
        elif number_teams > 1:
            for league, team in zip(d_league['leagues']['league'], d_team['teams']['team']):
                teams.append(
                    BasicTeam(league_name=league['name'], league_key=league['league_key'], team_name=team['name'],
...
class League:
    ...
    def _get_league_settings(self):
        ...
        roster_list = (settings['league']['settings']['roster_positions']['roster_position'])
        self.roster_positions = [x['position'] for x in roster_list for n in range(int(x['count']))]
        self.stats = OrderedDict()
        for s in settings['league']['settings']['stat_categories']['stats']['stat']:
            self.stats[s['stat_id']] = (s['display_name'], s.get('is_only_display_stat', 0) != '1')
...
class Team:
    ...
    def update_roster(self, data):
        ...
        r = self.parent.oauth.session.put(url, msg, headers={'content-type': 'application/xml'})
...
class Player:
    ...
    def get_rank(self):
        ...
        try:
            return int(float(data['player']['draft_analysis']['average_pick']))
        except ValueError:
            return 700

    def __repr__(self):
        return '<Player: {:<4} - {} ({})>'.format(self.selected_position, self.name['full'], self.position)
